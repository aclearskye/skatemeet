...
// Insert-only, so a store a user has already annotated is never clobbered by
// a later re-seed. store_metadata_osm_place_id_unique is a *partial* unique
// index (WHERE osm_place_id IS NOT NULL), which Postgres won't match against
// a plain `.upsert(..., { onConflict: "osm_place_id" })` — so instead we look
// up which place_ids already have a row and only insert the ones that don't.
async function seedStoreMetadata(allRows) {
  // A node matching multiple query clauses can appear twice — dedupe so a
  // single insert never conflicts with itself.
  ...
  for (let i = 0; i < toInsert.length; i += CHUNK) {
    ...
    const { error } = await supabase.from("store_metadata").insert(chunk);
...
// (a full 141-at-once burst would just get us rate-limited). Each lane still
...
async function processCity(city, index) {
  ...
  try {
    ...
    log.push(`  Found ${stores.length} stores`);
    await upsertStores(stores);
    await seedStoreMetadata(metadata);
    log.push(`  Saved ${stores.length}/${stores.length}`);
...
async function main() {
  if (isGlobal) {
    console.log(`\nGlobal store seed — querying ${CITIES.length} cities (r=${GLOBAL_RADIUS / 1000}km, concurrency=${CONCURRENCY})\n`);
