#!/usr/bin/env node
// Seed OSM skate spots into Supabase.
//
// Local area:  node scripts/seed-osm-spots.mjs --lat=45.5231 --lng=-122.6697 [--radius=10000]
// Global seed: node scripts/seed-osm-spots.mjs --global  (queries ~130 cities, ~7-10 min)
//
...
if (!isGlobal && (isNaN(lat) || isNaN(lng))) {
  console.error("Usage:");
  console.error("  node scripts/seed-osm-spots.mjs --global");
  console.error("  node scripts/seed-osm-spots.mjs --lat=<lat> --lng=<lng> [--radius=<metres>]");
  process.exit(1);
}

// ── Cities for global seed ────────────────────────────────────────────────────
// City-by-city radius queries are far more reliable than large bbox queries on
// the public Overpass API — bbox queries get silently throttled.
    ...
    try {
      ...
      const res = await fetch(mirror, {
        ...
        headers: {
          ...
          "User-Agent": "SkateMeet/1.0 (seed-script)",
        },
        body,
      });
...
async function upsertSpots(spots) {
  if (spots.length === 0) return;
  // Supabase upsert in chunks to avoid hitting request size limits
  const CHUNK = 500;
